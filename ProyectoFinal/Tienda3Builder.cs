...
{
    public class Tienda3Builder : PedidosBuilder
    {
        private Pedidos _pedido = new Pedidos();

        public override void PrepararInformacion()
        {
            string path = @"Jsons\JSONTienda3.json";
            string pathqr = @"Qrs\Tienda3.png";
            if (File.Exists(path))
            {
                File.Delete(path);
            }
            else if (File.Exists(pathqr))
            {
                File.Delete(pathqr);
            }
            _pedido.Check(1);
